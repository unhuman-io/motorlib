  ...
  void set_bias() {
    ...
    switch (startup_param_.motor_encoder_startup) {
      ...
      case StartupParam::ENCODER_BIAS_FROM_OUTPUT_WITH_MOTOR_CORRECTION: {
        ...
        logger.log_printf(
            "Output position: %f, motor_position: %f, motor mechanical "
            "position: %f",
            status.output_position, status.fast_loop.motor_position.position,
            status.fast_loop.motor_mechanical_position);
        logger.log_printf(
            "Encoder bias, round_by: %f, motor_bias_from_output: %f, "
            "motor_bias_rounded: %f",
