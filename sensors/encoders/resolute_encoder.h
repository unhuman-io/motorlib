...
#define RESOLUTE_SET_DEBUG_VARIABLES(prefix, api, resolute) \
    api.add_api_variable<APIUint32>(prefix "err", &resolute.diag_err_count_);\
    api.add_api_variable<APIUint32>(prefix "warn", &resolute.diag_warn_count_);\
    api.add_api_variable<APIUint32>(prefix "crc_cnt", &resolute.crc_err_count_);\
    api.add_api_variable<APIUint32>(prefix "raw", &resolute.raw_value_);\
    api.add_api_variable<const APICallback>(prefix "rawh",[](){ return u32_to_hex(resolute.raw_value_); });\
    api.add_api_variable<APIUint8>(prefix "len", &resolute.length_);\
    api.add_api_variable<const APIUint8>(prefix "ind", &resolute.byte_ind);\
    api.add_api_variable<const APIUint8>(prefix "crc_calc", &resolute.crc_calc_);\
    api.add_api_variable<const APIUint32>(prefix "zeros", &resolute.leading_zeros);\
    api.add_api_variable<const APIUint8>(prefix "diag", &resolute.diag_raw_.word);\
