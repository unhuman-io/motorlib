...
class APIInt : public APIVariable2<T> {
 public:
  ...
  void set(std::string s) { *this->value_ = std::stoi(s); }
};
...
class APIHex : public APIInt<T> {
 public:
  ...
  void set(std::string s) { *this->value_ = std::stoi(s, nullptr, 16); }
  virtual std::string get() const {
    std::vector<char> bytes((char *)this->value_,
                            (char *)this->value_ + sizeof(T));
...
class APICallback : public APIVariable {
 public:
  APICallback(std::function<std::string()> getfun,
              std::function<void(std::string)> setfun)
      : getfun_(getfun), setfun_(setfun) {}
...
class APICallbackFloat : public APIVariable {
 public:
  APICallbackFloat(std::function<float()> getfun,
                   std::function<void(float)> setfun)
      : getfun_(getfun), setfun_(setfun) {}
...
class APICallbackUint : public APIVariable {
 public:
  APICallbackUint(std::function<T()> getfun, std::function<void(T)> setfun)
      : getfun_(getfun), setfun_(setfun) {}
