...
// A circular stack. If data is written by one thread and read by one other
// thread then data is read from the top without worrying about thread safety.
...
class CStack {
 public:
  ...
  void finish() { pos_.store(future_pos_, std::memory_order_release); }
