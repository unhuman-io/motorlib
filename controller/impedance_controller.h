...
class ImpedanceController : public Controller {
 public:
  ImpedanceController(float dt)
      : Controller(dt), impedance_controller_(dt), torque_controller_(dt) {}
  ...
  float step(const MotorCommand &command, const MainLoopStatus &status) {
    float torque_des = impedance_controller_.step(command.position_desired,
