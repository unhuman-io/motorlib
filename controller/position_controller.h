...
class PositionController : public Controller {
 public:
  PositionController(float dt)
      : Controller(dt), controller_(dt), desired_filter_(dt) {}
  ...
  float step(const MotorCommand &command, const MainLoopStatus &status) {
    ...
    float iq_des = controller_.step(position_desired, command.velocity_desired,
                                    status.motor_position, velocity_limit_) +
