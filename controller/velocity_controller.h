...
class VelocityController : public Controller {
 public:
  VelocityController(float dt)
      : Controller(dt), controller_(dt), velocity_filter_(dt) {}
  void init(const MainLoopStatus &status) {
    controller_.init(
        0);  // can only switch into this mode at zero velocity without glitch
  ...
  float step(const MotorCommand &command, const MainLoopStatus &status) {
    float velocity_measured =
        wrap1_diff(status.motor_position, last_motor_position_, rollover_) /
        dt_;
    last_motor_position_ = status.motor_position;
    velocity_measured_filt_ = velocity_filter_.update(velocity_measured);
    float iq_des =
        controller_.step(command.velocity_desired, 0, velocity_measured_filt_,
                         acceleration_limit_) +
  ...
  void set_rollover(float rollover) {
    rollover_ = rollover;
    controller_.set_rollover(INFINITY);
  }
