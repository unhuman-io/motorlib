...
BoardPins get_board_pins(const BoardRev &board_rev) {
    ...
    if (board_rev.rev == BoardRev::Rev::kR1) {
        // led r pb 2
        board_pins.led_tim_r = &TIM5->CCR1; // af1
        // pc12 leg g
        board_pins.led_tim_g = &TIM5->CCR2; // af1
        // pf9 led b
        board_pins.led_tim_b = &TIM5->CCR4; // af6
    }
    return board_pins;
}
...
void pin_config_obot_g474_trace(const BoardRev &board_rev) {
     // Peripheral clock enable
        ...
        RCC->AHB2ENR |= RCC_AHB2ENR_GPIOAEN | RCC_AHB2ENR_GPIOBEN | RCC_AHB2ENR_GPIOCEN | RCC_AHB2ENR_GPIODEN | RCC_AHB2ENR_GPIOEEN | RCC_AHB2ENR_GPIOFEN | RCC_AHB2ENR_ADC12EN | RCC_AHB2ENR_ADC345EN;
        PWR->CR1 |= PWR_CR1_DBP;
    ...
    // Sleep mode enable
        ...
        GPIO_SETL(A, 4, GPIO_MODE::OUTPUT,  GPIO_SPEED::MEDIUM, 0);   // SPI1 CS drv8323s
        GPIO_SETL(A, 5, GPIO_MODE::ALT_FUN, GPIO_SPEED::MEDIUM, 5);   // SPI1 CLK drv8323s
        GPIO_SETL(A, 6, GPIO_MODE::ALT_FUN, GPIO_SPEED::MEDIUM, 5);   // SPI1 HIDO (host in device out) drv8323s
        GPIO_SETL(A, 7, GPIO_MODE::ALT_FUN, GPIO_SPEED::MEDIUM, 5);   // SPI1 HODI drv8323s
        ...
        // R1
        GPIO_SETL(B, 2, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH, 2);      // TIM5 CH1
        GPIO_SETH(C, 12, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH, 1);     // TIM5 CH2
        GPIO_SETH(F, 9, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH, 6);      // TIM5 CH4
        TIM5->CCMR1 = TIM_CCMR1_OC1PE | 6 << TIM_CCMR1_OC1M_Pos |   // preload and pwm mode 1
                        TIM_CCMR1_OC2PE | 6 << TIM_CCMR1_OC2M_Pos;
        TIM5->CCMR2 = TIM_CCMR2_OC4PE | 6 << TIM_CCMR2_OC4M_Pos;
        TIM5->CCER = TIM_CCER_CC1E | TIM_CCER_CC2E | TIM_CCER_CC4E; // enable
        TIM5->ARR = 0xFFFF;
        ...
        *etmteevr = 0x000037ef; // ON
        ...
        ADC1->SMPR2 = 2 << ADC_SMPR2_SMP13_Pos | // 12.5 cycles vbus, 294 ns, 200 ns min for opamp1
                      6 << ADC_SMPR2_SMP16_Pos | // 247.5 cycles interal temperature, 5.8us, 5us minup
