...
bool is_mps_driver_faulted() { return !(GPIOC->IDR & GPIO_IDR_ID14); }

void pin_config_obot_g474_osa() {
  // Peripheral clock enable
  RCC->APB1ENR1 =
      RCC_APB1ENR1_SPI3EN | RCC_APB1ENR1_TIM2EN | RCC_APB1ENR1_TIM4EN |
      RCC_APB1ENR1_TIM5EN | RCC_APB1ENR1_USBEN | RCC_APB1ENR1_RTCAPBEN |
      RCC_APB1ENR1_PWREN | RCC_APB1ENR1_I2C1EN | RCC_APB1ENR1_I2C2EN;
  ...
  // as qep
  // GPIO_SETL(A, 0, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH, 1);   // QEPA
  // TIM2 GPIO_SETL(A, 1, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH, 1);   //
  // QEPB TIM2 GPIO_SETL(A, 2, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH, 1); //
  // QEPI TIM2

  // i2c/scope
  GPIO_SETH(A, 8, GPIO_MODE::OUTPUT, GPIO_SPEED::VERY_HIGH,
            0);  // i2c2_sda as gpio
  GPIO_SETH(A, 9, GPIO_MODE::OUTPUT, GPIO_SPEED::VERY_HIGH,
            0);  // i2c2_scl as gpio
  GPIO_SETL(C, 5, GPIO_MODE::OUTPUT, GPIO_SPEED::VERY_HIGH,
            0);  // scope_3 as gpio

  // spi 1
  GPIO_SETL(A, 4, GPIO_MODE::OUTPUT, GPIO_SPEED::MEDIUM, 0);   // SPI1 CS
  GPIO_SETL(A, 5, GPIO_MODE::ALT_FUN, GPIO_SPEED::MEDIUM, 5);  // SPI1 CLK
  GPIO_SETL(A, 6, GPIO_MODE::ALT_FUN, GPIO_SPEED::MEDIUM,
            5);  // SPI1 HIDO (host in device out)
  ...
  GPIO_SETH(B, 15, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH,
            13);  // hrtim1 chd2
  GPIO_SETL(C, 6, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH,
            13);  // hrtim1 chf1
  GPIO_SETL(C, 7, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH,
            13);                                                  // hrtim1 chf2
  ...
  TIM4->CCMR1 = TIM_CCMR1_OC1PE |
                6 << TIM_CCMR1_OC1M_Pos |  // preload and pwm mode 1
  ...
  static_assert(CPU_FREQUENCY_HZ / config::main_loop_frequency / 4 < 65536,
  ...
  ADC12_COMMON->CCR = ADC_CCR_VSENSESEL | ADC_CCR_VREFEN |
                      3 << ADC_CCR_CKMODE_Pos;  // hclk/4 (42.5 MHz)
  ADC1->SQR1 = 13 << ADC_SQR1_SQ1_Pos;          // vbus on opamp
  ADC1->JSQR = 2 << ADC_JSQR_JL_Pos | 16 << ADC_JSQR_JSQ1_Pos |
               18 << ADC_JSQR_JSQ2_Pos |
               9 << ADC_JSQR_JSQ3_Pos;  // internal temperature, vrefint, ibus

  ADC1->CFGR = ADC_CFGR_JQDIS | ADC_CFGR_OVRMOD | 1 << ADC_CFGR_EXTEN_Pos |
               21 << ADC_CFGR_EXTSEL_Pos;  // trigger 21 -> hrtim trig1
  ...
  ADC1->SMPR2 = 2 << ADC_SMPR2_SMP13_Pos |  // 12.5 cycles vbus, 294 ns, 200 ns
                                            // min for opamp1
                6 << ADC_SMPR2_SMP16_Pos |  // 247.5 cycles interal
  ...
  ADC2->JSQR = 2 << ADC_JSQR_JL_Pos | 6 << ADC_JSQR_JSQ1_Pos |
               7 << ADC_JSQR_JSQ2_Pos | 8 << ADC_JSQR_JSQ3_Pos |
  ...
  ADC3->JSQR = 1 << ADC_JSQR_JEXTEN_Pos | 27 << ADC_JSQR_JEXTSEL_Pos |
  ...
  OPAMP1->CSR = 1 << OPAMP_CSR_VPSEL_Pos | 3 << OPAMP_CSR_VMSEL_Pos |
                ...
                OPAMP_CSR_OPAMPxEN;  // internal follower on pa3/vinp1
  ...
  SYSCFG->CFGR1 |= SYSCFG_CFGR1_I2C1_FMP | SYSCFG_CFGR1_I2C2_FMP |
                   SYSCFG_CFGR1_I2C_PB9_FMP;  // fast mode plus (1 MHz)
