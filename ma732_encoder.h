...
class MA732Encoder : public SPIEncoder {
 public:
  ...
  MA732Encoder(SPI_TypeDef &regs, GPIO &gpio_cs, uint8_t filter = 119,
               volatile int *register_operation = nullptr)
      : SPIEncoder(regs, gpio_cs), filter_(filter), regs_(regs) {
...
#ifdef STM32F446xx
      regs_.CR1 = SPI_CR1_MSTR | (3 << SPI_CR1_BR_Pos) | SPI_CR1_SSI |
                  SPI_CR1_SSM | SPI_CR1_SPE |
                  SPI_CR1_DFF;  // baud = clock/16, 16 bit
#else
  ...
  void set_bct(uint32_t value) { set_register(0x2, value); }

  uint32_t get_bct() { return read_register(0x2); }

  void set_et(uint32_t value) { set_register(0x3, value); }

  uint32_t get_et() { return read_register(0x3); }
  ...
  // you to set high and low thresholds in the 0x6 MGT register, then you can
  // read the 0x1B status register to determine if the field is within those
  // thresholds or not. The full range of the MGT register is 20 to 126 mT in 8
  // steps. Recommended min mT is 40 which is step 2 in MGT. I combine the two
  // readings like this (mght << 0 | (uint16_t) mglt << 8), so the minimum
  // recommended value is about 0x202 or 514.
  uint32_t get_magnetic_field_strength() {
    uint8_t original_mgt = read_register(0x6);
    uint8_t mght = 0, mglt = 0;
    for (uint8_t i = 0; i < 8; i++) {
      if (!set_register(
              0x6, i << 2 | i << 5)) {  // increment magnetic field thresholds
