...
void SPIEncoder::trigger() {
  ...
  // SPI is full duplex, to read you first write to the data register, then
  // wait, then read the data register
  regs_.DR = 0;
}

int32_t SPIEncoder::read() {
  // wait until receive not empty flag SPI_FLAG_RXNE is set, ma732 max frequency
  // 25 Mbps, 640 ns (115 cycles at 180 MHz)
  while (!(regs_.SR & SPI_SR_RXNE))
    ;  // RXNE: 1 -> data available
  data_ = regs_.DR;

  // some devices need a time delay between chip select and clock, 25 ns ma732,
  // 200 ns AEAT-8800
...
int32_t SPIEncoder::get_value() const { return data_; }
