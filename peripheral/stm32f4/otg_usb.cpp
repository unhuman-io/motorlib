...
void USB_OTG::send_data32(uint8_t endpoint, const uint32_t *data,
                          uint8_t length32, uint8_t length8) {
  ...
  if (endpoint != 0) {
    if (USBx_INEP(endpoint)->DIEPCTL & USB_OTG_DIEPCTL_EPENA) {
      ...
      } while (!(USBx_INEP(endpoint)->DIEPINT & USB_OTG_DIEPINT_INEPNE) &&
      ...
      while ((USBx->GRSTCTL & USB_OTG_GRSTCTL_TXFFLSH) &&
             !(USBx_DEVICE->DSTS & USB_OTG_DSTS_SUSPSTS) &&
             !(USBx->GRSTCTL & USB_OTG_GRSTCTL_AHBIDL))
        ;
      USBx->GRSTCTL =
          (USB_OTG_GRSTCTL_TXFFLSH |
           (uint32_t)(1 << (USB_OTG_GRSTCTL_TXFNUM_Pos + endpoint - 1)));
      while ((USBx->GRSTCTL & USB_OTG_GRSTCTL_TXFFLSH) &&
             !(USBx_DEVICE->DSTS & USB_OTG_DSTS_SUSPSTS))
        ;  // wait on flush
    }
  ...
  USBx_INEP(endpoint)->DIEPTSIZ =
      length8 | (((length8 - 1) / 64 + 1) << USB_OTG_DIEPTSIZ_PKTCNT_Pos);
    ...
    case 0x80:  // standard request get
      switch (setup_data[1]) {
        case 0x00:  // get status
          send_data(0, reinterpret_cast<const uint8_t *>("\x1\x0"),
                    2);  // self powered
          break;
          ...
          switch (setup_data[3]) {
            case 0x01:  // device descriptor
              send_data(0, USB_DEVICE_DESCIPTOR,
                        std::min(static_cast<size_t>(setup_data[6]),
