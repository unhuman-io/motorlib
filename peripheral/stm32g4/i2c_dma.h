...
// safe to call only at one time, i.e. don't call from interrupt and main
// simultaneously
class I2C_DMA {
 public:
  I2C_DMA(I2C_TypeDef &regs, DMA_Channel_TypeDef &tx_dma,
          DMA_Channel_TypeDef &rx_dma, uint16_t speed_khz = 100)
  ...
  int async_write(uint8_t address, int8_t nbytes, uint8_t *data,
                  bool stop = false) {
    if (!(regs_.CR2 & I2C_CR2_AUTOEND)) {
      // this will be new or a repeat start, still need to wait for dma to be
      // complete
      if (tx_dma_.CNDTR != 0) {
        return 1;
      }
    ...
    tx_dma_.CCR =
        DMA_CCR_EN | DMA_CCR_MINC | DMA_CCR_DIR;  // DIR = 1 > read from memory
    regs_.CR2 = (address << 1) | (nbytes << I2C_CR2_NBYTES_Pos) |
                (stop ? I2C_CR2_AUTOEND : 0) | I2C_CR2_START;
    return 0;
  }
  // return 1 for not ready, 0 for success
  int async_read(uint8_t address, uint8_t nbytes, uint8_t *data) {
    ...
    regs_.CR2 = (address << 1) | I2C_CR2_RD_WRN |
                (nbytes << I2C_CR2_NBYTES_Pos) | I2C_CR2_AUTOEND |
                I2C_CR2_START;
    return 0;
  }
  ...
  int write(uint8_t address, int8_t nbytes, uint8_t *data, bool stop = false,
  ...
  // return <= 0 for error, nbytes for success
  int read(uint8_t address, int8_t nbytes, uint8_t *data,
           uint16_t timeout_us = 1000) {
    bool error;
    uint32_t t_start = get_clock();
    bool timeout = false;
