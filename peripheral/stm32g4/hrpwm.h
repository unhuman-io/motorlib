...
class HRPWM : public PWMBase {
 public:
  ...
  void set_frequency_hz(uint32_t frequency_hz, uint16_t min_off_ns = 0,
                        uint16_t min_on_ns = 0, bool keep_prescaler = false);
...
class HRPWM3 : public HRPWM {
 public:
  HRPWM3(uint32_t frequency_hz, HRTIM_TypeDef &regs, uint8_t ch_a, uint8_t ch_b,
      ...
      : HRPWM(regs, regs.sTimerxRegs[ch_a].CMP1xR,
              regs.sTimerxRegs[ch_b].CMP2xR, regs.sTimerxRegs[ch_c].CMP1xR) {
    ch_a_ = ch_a;
    ch_b_ = ch_b;
    ch_c_ = ch_c;
    pwm3_mode_ = true;
