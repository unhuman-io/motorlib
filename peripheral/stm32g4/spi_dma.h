...
class SPIDMA {
 public:
  SPIDMA(SPI_TypeDef &regs, GPIO &gpio_cs, DMA_Channel_TypeDef &tx_dma,
         DMA_Channel_TypeDef &rx_dma, uint16_t start_cs_delay_ns = 100,
      ...
      : regs_(regs),
        ...
        tx_dma_(tx_dma),
        rx_dma_(rx_dma),
        start_cs_delay_ns_(start_cs_delay_ns),
        end_cs_delay_ns_(end_cs_delay_ns) {
  ...
  void readwrite(const uint8_t *const data_out, uint8_t *const data_in,
                 uint8_t length) {
    start_readwrite(data_out, data_in, length);
    finish_readwrite();
  }

  void start_readwrite(const uint8_t *const data_out, uint8_t *const data_in,
                       uint8_t length) {
