...
class QIA128_UART : public TorqueSensorBase {
 public:
  ...
  bool init() {
    ...
    uint8_t crc[] = {0x7b, 0x81, 0x87, 0x8D, 0x93, 0x99, 0x9F,
                     0xA5, 0xAB, 0xB1, 0xB7, 0xBD, 0xC3, 0xC9};
    ...
    logger.log_printf("qia128 offset: %d, full scale: %d", offset_,
                      full_scale_);
  ...
  float read() {
    if (state_ == WAIT) {
      ...
      if (regs_.ISR & USART_ISR_RXFT) {
        ...
        torque_ = (float)((int32_t)raw_ - (int32_t)offset_) /
                      (full_scale_ - offset_) * gain_ +
                  bias_;
        state_ = WAIT;
      }
    }
...
uint8_t crc8(uint8_t *p) { return p[0] * 1 + p[1] * 2 + p[2] * 3; }
